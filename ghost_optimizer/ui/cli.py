...
"""
    ...
    def apply_all(self):
        if not Confirm.ask(f"[bold {ACCENT}]Вы уверены, что хотите применить все твики?[/]"):
            self.status_message = "Действие отменено пользователем"
    ...
    def revert_all(self):
        if not Confirm.ask(f"[bold {ERR}]Вы уверены, что хотите отменить все твики?[/]"):
            self.status_message = "Действие отменено пользователем"
            self.status_style = MUTED
            return
