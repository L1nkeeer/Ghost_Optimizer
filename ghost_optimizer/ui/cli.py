...
"""
    ...
    def apply_all(self):
        if not Confirm.ask(f"[bold {ERR}]ВНИМАНИЕ:[/] Вы уверены, что хотите применить ВСЕ твики?"):
            self.status_message = "Отменено пользователем"
    ...
    def revert_all(self):
        if not Confirm.ask(f"[bold {ERR}]ВНИМАНИЕ:[/] Вы уверены, что хотите отменить ВСЕ твики?"):
            self.status_message = "Отменено пользователем"
            self.status_style = MUTED
            return
